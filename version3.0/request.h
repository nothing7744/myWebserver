#ifndef REQUEST
#define REQUEST

#include <pthread.h>
#include<string>
#include<unordered_map>
#include<memory>
using namespace std;

const int STATE_PARSE_URI = 1;
const int STATE_PARSE_HEADERS = 2;
const int STATE_RECV_BODY = 3;
const int STATE_ANALYSIS = 4;
const int STATE_FINISH = 5;
const int TIMER_TIME_OUT = 500;
const int MAX_BUFF = 1024;
const std::string PATH = "/";

// 有请求出现但是读不到数据,可能是Request Aborted,
// 或者来自网络的数据没有达到等原因,
// 对这样的请求尝试超过一定的次数就抛弃
const int AGAIN_MAX_TIMES = 200;

const int PARSE_URI_AGAIN = -1;
const int PARSE_URI_ERROR = -2;
const int PARSE_URI_SUCCESS = 0;

const int PARSE_HEADER_AGAIN = -1;
const int PARSE_HEADER_ERROR = -2;
const int PARSE_HEADER_SUCCESS = 0;

const int ANALYSIS_ERROR = -2;
const int ANALYSIS_SUCCESS = 0;

const int METHOD_POST = 1;
const int METHOD_GET = 2;
const int HTTP_10 = 1;
const int HTTP_11 = 2;

const int EPOLL_WAIT_TIME = 500;
struct mytimer;
class requestData;

void acceptConnection(int listen_fd, int epoll_fd);
//void handle_events(int epoll_fd, int listen_fd, struct epoll_event* events, int events_num,threadpool* tp);
void handle_expired_event();
void myHandler(shared_ptr<void>args);

class MimeType
{
private:
    static pthread_mutex_t mlock;
    static std::unordered_map<std::string, std::string> mime;
    MimeType();
    MimeType(const MimeType &m);
public:
    static std::string getMime(const std::string &suffix);
};

struct mytimer
{
    bool deleted;
//size_t表示的是无符号整数,在32位系统是unsigned int,在64位系统是unsigned long
    size_t expired_time;
//    requestData *request_data;
    shared_ptr<requestData> request_data;

    mytimer(shared_ptr<requestData> _request_data, int timeout);
    ~mytimer();
    void update(int timeout);
    bool isvalid();
    void clearReq();
    void setDeleted();
    bool isDeleted() const;
    size_t getExpTime() const;
};

enum HeadersState
{
    h_start = 0,
    h_key,
    h_colon,
    h_spaces_after_colon,
    h_value,
    h_CR,
    h_LF,
    h_end_CR,
    h_end_LF
};

//在这里这么定义，使用enable_shared_from_this之后才可以使用shared_from_this()函数
 class requestData: public std::enable_shared_from_this<requestData>
{
private:
    int againTimes;
    std::string path;
    int fd;
    int epollfd;
    // content的内容用完就清
    std::string content;
    int method;
    int HTTPversion;
    std::string file_name;
    int now_read_pos;
    int state;
    int h_state;
    bool isfinish;
    bool keep_alive;
    std::unordered_map<std::string, std::string> headers;
    //mytimer *timer;
    shared_ptr<mytimer>timer;


private:
     int parse_URI();
     int parse_Headers();
     int analysisRequest();


public:
    requestData();
    requestData(int _epollfd, int _fd, std::string _path);
    int getFd();
    //void addTimer(mytimer *mtimer);
    void addTimer(shared_ptr<mytimer>mtimer);
    void reset();
    void seperateTimer();
    void setFd(int fd);
    ~requestData();
    void handleRequest();
    void handleError(int fd, int err_num, std::string short_msg);
};

struct timerCmp
{
    bool operator()(std::shared_ptr<mytimer> a, std::shared_ptr<mytimer> b) const;
};


/*struct timerCmp
{
    bool operator()(shared_ptr<mytimer> &a, shared_ptr<mytimer> &b);
};*/


class MutexLockRAII
{
public:
//构造函数 
     MutexLockRAII();
     MutexLockRAII(pthread_mutex_t qlock);
//析构函数
    ~MutexLockRAII();
private:
    static pthread_mutex_t qlock;
private:
//拷贝函数
    MutexLockRAII(const MutexLockRAII&);
//操作符
    MutexLockRAII& operator=(const MutexLockRAII&);
};
#endif
