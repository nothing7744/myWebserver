...
pthread_mutex_t MutexLockRAII::qlock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t MimeType::mlock = PTHREAD_MUTEX_INITIALIZER;

std::unordered_map<std::string, std::string> MimeType::mime;
//资源的媒体类型(把先前的格式转换为mime的格式)
...
void requestData::handleRequest()
{
...

    {
         ...
         //   shared_ptr<mytimer>mtimer(make_shared<mytimer>(this,500));
            shared_ptr<mytimer>mtimer(make_shared<mytimer>(shared_from_this(),500));
...
//    int ret = Epoll::epoll_mod(epollfd, fd, this, _epo_event);
    int ret = Epoll::epoll_mod(epollfd, fd, shared_from_this(), _epo_event);
    if (ret < 0)
    {
        // 返回错误处理
        //delete this;
...
//之后对我们得到的请求进行处理(解析完全的get和post请求)
...
//get产生一个tcp数据包，post产生两个tcp数据包
    if (method == METHOD_POST)
    {
        ...
        Mat test = imdecode(data, cv::IMREAD_ANYDEPTH| cv::IMREAD_GRAYSCALE);
    ...
    //struct stat这个结构体是用来描述一个系统文件系中文件属性的结构,执行成功时返回0
        struct stat sbuf;
...
//        requestData *req_info = new requestData(epoll_fd, accept_fd,PATH);
        shared_ptr<requestData>req_info(make_shared<requestData>(epoll_fd, accept_fd,PATH));
        ...
        Epoll::epoll_add(epoll_fd, accept_fd, req_info,_epo_event);
        // 新增时间信息
        //mytimer *mtimer = new mytimer(req_info, TIMER_TIME_OUT); make_shared类似于new表示一个新建的对象
         shared_ptr<mytimer>mtimer(make_shared<mytimer>(req_info,TIMER_TIME_OUT));
...
void myHandler(shared_ptr<void>args)
{
    //requestData *req_data = (requestData*)args;
    shared_ptr<requestData>req_data=std::static_pointer_cast<requestData>(args);
    //shared_ptr<requestData>req_data(args);
