...
//共享资源需要加锁
void*worker(void*threadpl){
    threadpool*pool=(threadpool*)threadpl;
    task Task;
    while(1){
    ...
    Task.fun=pool->taskQ[pool->first].fun;
    Task.args=pool->taskQ[pool->first].args;
...
int threadpool_add(threadpool* pool,function<void(shared_ptr<void>args)> func,shared_ptr<void>args){
    pthread_mutex_lock(&(pool->mutex_pool));
    ...
    pool->taskQ[pool->tail].fun=func;
    pool->taskQ[pool->tail].args=args;
